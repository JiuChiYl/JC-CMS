...
interface FireworksConfig {
  ...
  circleRadius: { min: number, max: number }
  diffuseRadius: { min: number, max: number }
  animeDuration: { min: number, max: number }
}
...
function setParticleDirection(p: { x: number, y: number }): { x: number, y: number } {
...
const originalApplyUserProperties = window.wallpaperPropertyListener.applyUserProperties.bind(window.wallpaperPropertyListener)

window.wallpaperPropertyListener.applyUserProperties = function (props: Record<string, { value: unknown }>) {
  ...
  if (props.fireworkstheme) {
    updateTheme(props.fireworkstheme.value as string)
  }
}

const originalSetPaused = window.wallpaperPropertyListener.setPaused.bind(window.wallpaperPropertyListener)

window.wallpaperPropertyListener.setPaused = function (isPaused: boolean) {
