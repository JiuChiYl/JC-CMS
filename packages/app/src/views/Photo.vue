...
import { Search, Download, Delete, ArrowDown } from '@element-plus/icons-vue'
...
const filterArr = reactive<FilterItem[]>([])
const imgArr = reactive<ImgItem[]>([])
...
const handleCommand = (command: FilterChild, type: string) => {
  filterArr.forEach((item) => {
    if (item.type === type) {
...
const handlePictureCardPreview = (_file: never) => {}
const handleRemove = (_file: never) => {}
            ...
            <div class="search">
              ...
              <div v-if="filterFlag" class="filter-sty">
                <el-dropdown v-for="item in filterArr" :key="item.type" class="dropdown-sty"
                  @command="(command: FilterChild) => handleCommand(command, item.type)">
